...
class Vector3(object):
    ...
    def __repr__(self):
        ...
        return "%s(%f, %f, %f)" % (
        self.__class__.__name__, self.values[0], self.values[1], self.values[2])
...
class Quaternion(object):
    ...
    def __repr__(self):
        ...
        return "%s(%f, %f, %f, %f)" % (
        self.__class__.__name__, self.values[0], self.values[1], self.values[2], self.values[3])
    ...
    def _quaternion_length(q_):
        ...
        """
        return math.sqrt(
            (q_[QX] * q_[QX]) + (q_[QY] * q_[QY]) + (q_[QZ] * q_[QZ]) + (q_[QW] * q_[QW]))
    ...
    def toMatrix3(self):
        ...
        norm = input[QX] * input[QX] + input[QY] * input[QY] + input[QZ] * input[QZ] + input[QW] * \
               input[QW]
