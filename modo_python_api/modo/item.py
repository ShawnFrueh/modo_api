...
class Item(object):
    ...
    def index(self):
        ...
        """
        for x in xrange(self.scene.ItemCount(self.Type())):
    ...
    def setParent(self, newParent=None, index=None):
        ...
        """
        if isinstance(newParent, basestring):
    ...
    def rootIndex(self):
        ...
        for x in xrange(graph.RootCount()):
    ...
    def __descendants(self, itemType, asSubType=True):
        ...
        while not len(queue) == 0:
            a = queue.pop()
            if a not in items and (
                    itemType is None or (itemType is not None and a.Type() == itemType)):
                items.append(a)
            queue += a.SubList()
    ...
    def children(self, recursive=False, itemType=None, asSubType=True):
        ...
        """
        if itemType and isinstance(itemType, basestring):
        ...
        if itemType:
            ...
            return [util.typeToFunc(item.Type())(item) for item in self.SubList() if
                    item.Type() == itemType]
    ...
    def childrenByType(self, itype, asSubType=True):
        ...
        """
        if isinstance(itype, basestring):
    ...
    def getTags(self, values=True):
        ...
        """
        tags = {} if values else []
        for x in xrange(self._stringtag.Count()):
    ...
    def itemGraphNames(self):
        ...
        for i in xrange(self.scene.GraphCount()):
    ...
    def packageNames(self):
        ...
        for i in xrange(hst_svc.NumServers(lx.symbol.a_PACKAGE)):
    ...
    def deformers(self, dtype=None):
        ...
        """
        if dtype:
            return tuple(deformer for deformer in self.itemGraph('deformers').reverse() if
                         deformer.type == dtype)
...
class TransformItem(Item):
    ...
    def __transformType(self):
        ...
        for transformType, name in TransformItem.__types.iteritems():
...
class Camera(LocatorSuperType):
    ...
    def __init__(self, camera=None):
        ...
        # the correct type.
        if camera:
            if isinstance(camera, basestring):
...
class Locator(LocatorSuperType):
    ...
    def __init__(self, locator=None):
        ...
        if locator:
            ...
            if not isinstance(locator,
                              (lx.object.Item, Item)) or not locator.Type() == c.LOCATOR_TYPE:
...
class GroupLocator(LocatorSuperType):
    ...
    def __init__(self, grploc=None):
        ...
        # the correct type.
        if grploc:
            if isinstance(grploc, basestring):
                grploc = scene.Scene().ItemLookup(grploc)
            if not isinstance(grploc,
                              (lx.object.Item, Item)) or not grploc.Type() == c.GROUPLOCATOR_TYPE:
...
class TextureLocator(LocatorSuperType):
    ...
    def __init__(self, texloc=None):
        ...
        # the correct type.
        if texloc:
            if isinstance(texloc, basestring):
                texloc = scene.Scene().ItemLookup(texloc)
            if not isinstance(texloc,
                              (lx.object.Item, Item)) or not texloc.Type() == c.TXTRLOCATOR_TYPE:
...
class Light(LocatorSuperType):
    ...
    def __init__(self, light):
        ...
        if light.Type() == c.LIGHT_TYPE:
            raise TypeError(
                "supplied item was a 'Light' supertype, a specific light type is required.")
    ...
    def material(self):
        ...
        for x in xrange(parentGraph.RevCount(self)):
...
class CylinderLight(Light):
    ...
    def __init__(self, light=None):
        ...
        if light:
            if isinstance(light, basestring):
                light = scene.Scene().ItemLookup(light)
            if (isinstance(light, (lx.object.Item, Item))) and (
                    light.Type() != cylinder_light_type):
...
class Group(Item):
    ...
    def __init__(self, groupItem=None, gtype=''):
        if groupItem:
            if isinstance(groupItem, basestring):
                groupItem = scene.Scene().ItemLookup(groupItem)
            if (isinstance(groupItem, (lx.object.Item, Item))) and not groupItem.TestType(
                    c.GROUP_TYPE):
        ...
        self._channel_graph = lx.object.ChannelGraph(
            self.scene.GraphLookup(lx.symbol.sGRAPH_CHANGROUPS))
    ...
    def items(self):
        ...
        """
        item_list = [self._itemGraph.FwdByIndex(self, x) for x in
                     xrange(self._itemGraph.FwdCount(self))]
    ...
    def groupChannels(self):
        ...
        """
        channels = []
        for x in xrange(self._channel_graph.FwdCount(self, -1)):
...
class RenderPassGroup(Group):
    ...
    def passes(self):
        ...
        """
        p = []
        for x in xrange(self._itemGraph.FwdCount(self)):
            try:
...
class ActionClip(Item):
    ...
    def __init__(self, clip):
        ...
        if clip.Type() not in (
        c.SCENE_SVC.ItemTypeLookup('actionclip'), c.SCENE_SVC.ItemTypeLookup('actionpose')):
    ...
    def passGroup(self):
        ...
        for x in xrange(itemGraph.RevCount(self)):
...
class ItemGraph(object):
    ...
    def __repr__(self):
        return "modo.%s('%s', '%s')" % (
        self.__class__.__name__, self._item.Ident(), self._graphType)
    ...
    def _items(self, index=None, asSubType=True):
        if index is not None and isinstance(index, (int, long)):
            if not asSubType:
            ...
            if not asSubType:
                result = [Item(self._indexFunc(self._item, i)) for i in xrange(len(self))]
            else:
                result = [self[i] for i in xrange(len(self))]
        return result
    ...
    def connectedItems(self):
        ...
        return {key: value for key, value in
                zip(('Reverse', 'Forward'), (self.reverse(), self.forward()))}
    ...
    def __lshift__(self, other):
        ...
        """
        if not isinstance(other, ItemGraph) and not (
                hasattr(other, '_item') and other._item.test()):
