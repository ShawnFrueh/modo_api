...
class Channel(object):
    ...
    def name(self):
        ...
        """
        if isinstance(self._chan, (int, long,)):
    ...
    def graph(self):
        ...
        if self._graph is None:
            ...
            if hasattr(self.item, 'scene'):
                self._graph = lx.object.ChannelGraph(
                    self.item.scene.GraphLookup(lx.symbol.sGRAPH_CHANLINKS))
            else:
                self._graph = lx.object.ChannelGraph(
                    self.item.Context().GraphLookup(lx.symbol.sGRAPH_CHANLINKS))
        return self._graph
    ...
    def fwdLinked(self):
        ...
        """
        linked = []
        for x in xrange(self.graph.FwdCount(self.item, self.index)):
    ...
    def revLinked(self):
        ...
        """
        linked = []
        for x in xrange(self.graph.RevCount(self.item, self.index)):
    ...
    def setLink(self, from_index, to_channel, to_index):
        ...
        """
        self.graph.SetLink(self.item, self.index, from_index, to_channel.item, to_channel.index,
                           to_index)
...
class ChannelTriple(object):
    ...
    def __init__(self, item, channelName):
        ...
        if extension:
            self._channels = [item.channel('%s.%s' % (channelName, extension)) for extension in
                              extension]
            return
