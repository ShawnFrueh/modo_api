...
class MeshGeometry(object):
    ...
    def __new__(cls, item=None, mode='write'):
        ...
        for cachedItem, cachedGeometry in cls._cache.iteritems():
...
class MeshProvider(object):
    ...
    def meshFromMeshChannel(item, value='write', time=None):
        ...
        if value is "write":
            ...
            write_mesh_obj = chanWrite.ValueObj(item,
                                                item.ChannelLookup(lx.symbol.sICHAN_MESH_MESH))
...
class LayerScanItemEdit(object):
    ...
    def __init__(self, item, flags=lx.symbol.f_LAYERSCAN_EDIT):
        ...
        try:
            ...
            raise RuntimeError(
                "Layer scan allocation failed. Did you forget to call setMeshEdits earlier?")
...
class MeshVertex(object):
    ...
    def __repr__(self):
        ...
        return 'modo.%s(%s, \'%s\')' % (
        self.__class__.__name__, strIndexOrId, self._geometry._item.Ident())
    ...
    def remove(self):
        ...
        for poly_index in reversed(range(mesh.PolygonCount())):
            ...
            for vi in range(polygon.VertexCount()):
                pt = long(polygon.VertexByIndex(vi))
                if not pt == self._id:
                    vlist.append(pt)

            # Delete the polygon if it would end up with less than 3 points
            if len(vlist) < 3:
                MeshPolygons._performSelectionOperation(getIDAsLong(polygon), mesh,
                                                        operation='deselect')
                polygon.Remove()
                continue
    ...
    def __performSelectionOp(self, operation=0, polygon=None):
        ...
        MeshVertices._vertexSelectionOperation(getIDAsLong(self), self.provider.mesh, operation,
                                               polygon=polygon)
    ...
    def vertices(self):
        ...
        ids = tuple(long(self.PointByIndex(i)) for i in range(self.PointCount()))
    ...
    def polygons(self):
        ...
        point = self.provider.PointAccessor
        point.Select(self._id)
        ids = tuple(long(point.PolygonByIndex(i)) for i in range(point.PolygonCount()))
    ...
    def nextPointByVector(self, direction, minAlign=-1.0):
        ...
        elif isinstance(direction, (int, long)):
            index = direction
...
class MeshVertices(MeshComponentContainer):
    ...
    def __delitem__(self, index):
        if isinstance(index, (list, tuple)):
            ...
            if isinstance(verts[0], MeshVertex):
                for vert in verts:
                    vert.remove()
            elif isinstance(verts[0], (int, long)):
                indices = index
    ...
    def _vertexSelectionOperation(vertex_id, mesh, operation=0, polygon=None):
        ...
        if polygon is None:
            ...
            polygon = polygon.id
        elif isinstance(polygon, (int, long)):
    ...
    def __iter__(self):
        ...
        for i in xrange(self.provider.mesh.PointCount()):
    ...
    def selected(self):
        ...
        for vi in xrange(numVerts):
            ...
            vertexID = long(vTransPacket.Vertex(packetPointer))
        ...
        if isinstance(vertices, collections.Iterable) and len(vertices) > 0:
            ...
            if isinstance(vertices[0], int):
                for index in vertices:
                    ...
                    MeshVertices._vertexSelectionOperation(getIDAsLong(vertex), self.provider.mesh,
                                                           0)

            elif isinstance(vertices[0], long):
                for pid in vertices:
...
class MeshEdge(object):
    ...
    def fromVertices(cls, vertices, geometry, mesh=None):
        ...
        if isinstance(vertices, (list, tuple)):
            if len(vertices) <= 0:
                raise AttributeError("Empty list or tuple")
            if isinstance(vertices[0], (int, long)):
                point = geometry.provider.PointAccessor
                if not point.test():
                    return
                if isinstance(vertices[0], int):
    ...
    def vertex_indices(self):
        ...
        ids = tuple(long(p) for p in self._accessor.Endpoints())
    ...
    def __div__(self, other):
        """Divides by position"""
        self.__apply_operator("__div__", other)
    ...
    def polygons(self):
        ...
        ids = tuple(long(self.PolygonByIndex(i)) for i in range(self.PolygonCount()))
        return tuple(MeshPolygon.fromID(id, self._geometry) for id in ids)
...
class MeshEdges(MeshComponentContainer):
    ...
    def _performSelectionOperation(edge_accessor, mesh, operation='add'):
        ...
        vTransPacket = lx.object.EdgePacketTranslation(
            _edgeSelect.selsvc.Allocate(lx.symbol.sSELTYP_EDGE))
        ...
        id1, id2 = tuple(long(p) for p in edge_accessor.Endpoints())
    ...
    def __pollEdgeIDs(self):
        ...
        try:
            ...
            raise Exception(
                "This functionality is not available from the fire and forget python interpreter")
...
class MeshPolygon(object):
    ...
    def __init__(self, index, geometry):
        ...
        if not hasattr(MeshPolygon, '_polySelect'):
            ...
            MeshPolygon._polySelTypeCode = MeshPolygon._polySelect.selsvc.LookupType(
                lx.symbol.sSELTYP_POLYGON)
    ...
    def vertexNormal(self, index):
        ...
        """
        point = self.provider.point
        id = long(self._accessor.VertexByIndex(index))
        point.Select(id)
    ...
    def tags(self):
        ...
        for name, tagType in polytag.iteritems():
    ...
    def __lookupUVMap(self, uvmap):
        ...
        # If string
        elif isinstance(uvmap, basestring):
    ...
    def getUV(self, vertex, uvmap=None):
        ...
        vertex_id = vertex._id if isinstance(vertex, MeshVertex) else long(
            self._accessor.VertexByIndex(vertex))
    ...
    def select(self, replace=False):
        ...
        """
        self._performSelectionOperation('replace') if replace else self._performSelectionOperation(
            'add')
...
class MeshPolygons(MeshComponentContainer):
    ...
    def selected(self):
        ...
        vTransPacket = lx.object.PolygonPacketTranslation(
            selSrv.Allocate(lx.symbol.sSELTYP_POLYGON))

        numPolys = selSrv.Count(polySelTypeCode)
        poly_ids = []

        for vi in xrange(numPolys):
    ...
    def select(self, polygons=None, replace=False):
        ...
        if isinstance(polygons, (list, tuple)) and len(polygons) > 0:
            ...
            if isinstance(polygons[0], int):
                for index in polygons:
                    polygon = MeshPolygon(index, self._geometry)
                    MeshPolygons._performSelectionOperation(getIDAsLong(polygon),
                                                            self.provider.mesh, 'add')

            elif isinstance(polygons[0], long):
                for pid in polygons:
...
class VertexMap(object):
    ...
    @classmethod
    def fromMesh(cls, geometry, accessor, ID=None, map_type=lx.symbol.i_VMAP_RGBA, name=None,
                 **kwargs):
        instance = cls.__new__(cls)
        instance._geometry = geometry
...
class UVMap(VertexMap):
    ...
    def __get(self, point_index, listContinuous=True, listNonContinuous=True):
        ...
        elif isinstance(point_index, long):
        ...
        for polygon_index in xrange(point.PolygonCount()):
            ID = long(point.PolygonByIndex(polygon_index))
            poly.Select(ID)
    ...
    def _setValue(self, vertexIndex, values, nonContinuous=False):
        ...
        if not nonContinuous and num_polys >= 1:
            ...
            for polygon_index in xrange(num_polys):
...
class VertexNormalMap(VertexMap):
    ...
    * Available from Modo 901 SP1 and upwards *
    ...
    def setNormal(self, values, vertex, polygon=None, normalize=True):
        ...
        if isinstance(polygon, (int, long)):
            polygon = MeshPolygon(polygon, self._geometry)

        # Set the normal on the polygon if one was passed
        if isinstance(polygon, MeshPolygon):
            polygon.accessor.SetMapValue(getIDAsLong(vertex), getIDAsLong(self._accessor),
                                         self.storage)
    ...
    def getNormal(self, vertex, polygon=None):
        ...
        # Set the normal on the polygon if one was passed
        if isinstance(polygon, MeshPolygon):
            polygon.accessor.MapValue(getIDAsLong(self._accessor), getIDAsLong(vertex),
                                      self.storage)
        ...
        class QueryMapsVisitor(lxifc.Visitor):
            ...
            def vis_Evaluate(self):
                ...
                if name:
                    ...
                    vmap = {"name": name, "type_name": type_name, "ID": getIDAsLong(self.meshmap),
                            "map_type": map_type}
