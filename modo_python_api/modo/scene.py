...
def sceneList():
    ...
    return [Scene(rootCinema.SubSceneByIndex(c.MESH_TYPE, x)) for x in
            xrange(rootCinema.SubSceneCount(c.MESH_TYPE))]
...
class Scene(object):
    ...
    def selected(self):
        ...
        """
        return [util.typeToFunc(selected.Type())(selected) for selected in
                lxu.select.ItemSelection().current()]
    ...
    def deselect(self, itemObj=None):
        ...
        if isinstance(itemObj, basestring):
    ...
    def addJointLocator(self, name=None, parent=None):
        ...
        channelValues = {'lsOffsetS': 0.0, 'isRadius': 0.25, 'isAuto': 0L, 'isSize.Y': 1.0,
                         'lsShape': 'rhombus', 'isSize.X': 1.0, 'isSize.Z': 1.0, 'lsRadius': 0.5,
                         'lsWidth': 0.5,
                         'isOffset.Y': 0.0, 'isOffset.X': 0.0, 'isOffset.Z': 0.0, 'isAlign': 0L,
                         'isShape': 'sphere',
                         'isSolid': 0L, 'lsAuto': 1L, 'lsOffsetE': 0.0, 'isStyle': 'replace',
                         'lsSolid': 1L,
        ...
        lx.object.Locator(joint).ZeroTransform(self.chanRead, self.chanWrite,
                                               lx.symbol.iXFRM_ROTATION)
    ...
    def _allItems(self):
        ...
        """
        types = []
        for i in xrange(c.SCENE_SVC.ItemTypeCount()):
    ...
    def iterItemsFast(self, itype):
        ...
        for i in xrange(self.ItemCount(itype)):
    ...
    def actors(self):
        ...
        return [actor for actor in groups if ('GTYP', 'actor') in actor.getTags().items()]
