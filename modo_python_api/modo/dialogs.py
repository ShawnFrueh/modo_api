...
def customFile(dtype, title, names, unames, patterns=None, ext=None, path=None):
    ...
    for index, name in enumerate(names):
        if dtype == 'fileSave':
            cmd('dialog.fileTypeCustom {{{0:s}}} {{{1:s}}} "" {{{2:s}}}'.format(name, unames[index],
                                                                                ext[index]))
        else:
            cmd('dialog.fileTypeCustom {{{0:s}}} {{{1:s}}} {{{2:s}}} ""'.format(name, unames[index],
                                                                                patterns[index]))
    ...
    if valArray.Count() > 0:
        if dtype == 'fileOpenMulti':
            return [valArray.GetString(x) for x in xrange(valArray.Count())]
...
def okCancel(title, message):
    ...
    try:
        ...
        if e.message == 'bad result: MSGDIALOG_CANCEL':
            return 'cancel'
        else:
            raise
    return 'ok'
...
def yesNo(title, message):
    ...
    try:
        ...
        if e.message == 'bad result: MSGDIALOG_NO':
            return 'no'
        else:
            raise
    return 'yes'
...
def saveOK(title, message):
    ...
    try:
        ...
        if e.message == 'bad result: MSGDIALOG_CANCEL':
            return 'cancel'
        elif e.message == 'bad result: MSGDIALOG_NO':
            return 'no'
        else:
            raise
    return 'save'
...
def yesNoAll(title, message):
    ...
    try:
        ...
        if e.message == 'bad result: MSGDIALOG_CANCEL':
            ...
            return 'no'
        elif e.message == 'bad result: MSGDIALOG_NOTOALL':
            return 'notoall'
        else:
            raise
    return 'ok'
